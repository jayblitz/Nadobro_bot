...
- **Copy Trading backend infrastructure**: Nado-native copy trading (`copy_service.py`). A background **polling** loop (default 30s) loads each leader wallet’s open positions via `NadoClient`, compares to mirrored user positions, and executes proportional market orders / closes as needed. `hl_client.py` remains for **Hyperliquid REST** lookups where the UI shows HL equity/positions for a trader preview (not a live HL WebSocket). DB tables: `copy_traders`, `copy_mirrors`, `copy_trades`. Up to 5 active mirrors per user. Admin helpers in `admin_service.py`. Polling lifecycle is wired from the bot process when copy trading is enabled (`NADO_COPY_TRADING`, default true).
...
- **`/status` command UX overhaul**: Removed raw Perf Snapshot (p50/p95/avg ms developer metrics) from user-facing output. Redesigned status display: shows live uptime, next scan countdown, last action with detail, Alpha Agent trade count/PnL/open positions, error streak warnings, and cleaner onboarding section that only shows incomplete steps. Strategy states `last_action` and `last_action_detail` now persisted in bot state for both Alpha Agent and non-bro strategies. Status no longer shows confusing "COMPLETE"/"IDLE" — shows "NOT RUNNING" when stopped (with last action and stop reason) or "RUNNING" with live cycle info when active.
...
| `hl_client.py` | Async HTTP client for Hyperliquid info API (used for trader preview / equity & positions on HL) |
| `copy_service.py` | Copy trading: poll leader Nado positions, sync mirrors, proportional sizing, notifications |
...
- `_dispatch_strategy()` routes: grid/rgrid → `strategies/mm_bot.py`, dn → `strategies/delta_neutral.py`, vol → `strategies/volume_bot.py`, bro → no-op (Alpha Agent autoloop backend retired)
- SUPPORTED_STRATEGIES = ("grid", "rgrid", "dn", "vol", "bro")

### Alpha Agent — Autonomous LLM Quant Agent
...
- **Strategy cycle** (Alpha Agent strategy backend, since removed): 5-min cycle scans all assets, builds market snapshots (technicals + funding + CMC + X sentiment), sends to Grok-3 for structured decisions (open/close/hold/emergency_flatten), manages full position lifecycle
...
- **Anti-flip-flop Cooldowns**: `_check_cooldown()` in the Alpha Agent tracks `recent_closes` per product. If re-entering same direction within 3 cycles, requires confidence bump of +15%.
...
- **UI**: Alpha Agent card in strategy hub, config keyboard (with profile presets), risk preset picker, Why?/Game Plan explainability, HOWL approval flow, pending bro input handler in messages.py
