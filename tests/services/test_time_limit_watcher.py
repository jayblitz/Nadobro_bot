...
from src.nadobro.strategy.time_limit_watcher import _close_position, time_limit_tick


def test_time_limit_tick_enqueues_claimed_rows(monkeypatch):
    claimed = {"positions": [{"id": 1}], "orders": [{"id": 2}]}
    monkeypatch.setattr("src.nadobro.strategy.time_limit_watcher.time_limit_enabled", lambda: True)
    monkeypatch.setattr("src.nadobro.strategy.time_limit_watcher.fetch_due_time_limits", lambda now, network: claimed)
    enqueued = []
    ...
    monkeypatch.setattr("src.nadobro.strategy.time_limit_watcher.enqueue_strategy", fake_enqueue)
...
def test_time_limit_close_uses_claimed_row_network(monkeypatch):
    ...
    monkeypatch.setattr("src.nadobro.strategy.time_limit_watcher.execute_market_order", fake_execute_market_order)
